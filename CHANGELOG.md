...
- `TerminalSettings` model with appearance, behavior, and shell configuration
- `IFontService` interface for font detection and management
- `FontService` with lazy monospace font discovery and platform recommendations
- 40 unit tests (23 TerminalSettingsTests + 17 FontServiceTests)
...
- `TerminalSearchBarViewModel` with debounced search, navigation commands, option toggles
- `TerminalSearchBar.axaml` floating search bar with input, toggles, navigation buttons
- `TerminalSearchBar.axaml.cs` keyboard shortcuts (Enter, Shift+Enter, Escape, Alt+C, Alt+R, F3)
- Search result highlighting in `TerminalRenderer` using SkiaSharp
- 24 unit tests for TerminalSearchBarViewModel

### Extended

- `TerminalRenderer` with SetSearchState, SetHighlightStyle, ScrollToResult, RenderSearchHighlights
- `TerminalPanelViewModel` with SearchBarViewModel property and OpenSearchCommand
- `TerminalPanel.axaml` with TerminalSearchBar overlay
- `Dark.axaml` with search bar theme colors
...
- `TruncationMode` enum (KeepStart, KeepEnd, KeepBoth) with GetIndicator(), ToDescription() extensions
- `OutputCaptureSettings` with limits, truncation mode, ANSI stripping, line normalization, factory methods
- `IOutputCaptureService` with StartCapture/StopCapture, CaptureBuffer, CaptureSelection, history, configuration
- `OutputCaptureService` with output event subscription, source-generated ANSI regex, per-session history
- 28 unit tests covering capture, processing, truncation, and history


## [0.5.4c] - 2026-01-18

Command Execution Service. Executes commands in terminal sessions with clipboard and status tracking.

### Added

- `CommandStatusChangedEventArgs` with CommandId, OldStatus, NewStatus, SessionId, Timestamp and computed helpers
- `CommandExecutionResult` with execution status, timing, output capture, and error details
- `ICommandExecutionService` with CopyToClipboardAsync, SendToTerminalAsync, ExecuteAsync, ExecuteAllAsync, CancelExecutionAsync, GetStatus, EnsureTerminalSessionAsync
- `CommandExecutionService` implementation with status tracking, session reuse, and shell profile matching
- 21 unit tests covering clipboard, terminal, status, and session operations
